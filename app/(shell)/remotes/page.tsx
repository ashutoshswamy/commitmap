...
import { getRepo, getForks, parseRepoInput, type Repo } from "../../lib/github";
...
export default async function RemotesPage({
  ...
  try {
        ...
        <main className="flex-1 min-h-0 overflow-auto bg-surface-container-lowest">
          <div className="max-w-5xl mx-auto px-6 lg:px-8 py-8 lg:py-12">
            <header className="mb-12">
              <h1 className="font-headline font-bold text-3xl text-on-surface tracking-tight mb-2">Network</h1>
              <p className="text-on-surface-variant/70 text-sm">Explore the upstream project and community forks.</p>
            </header>
            ...
            <div className="space-y-12">
              ...
              {(repo.parent || repo.source) && (
                <section>
                  <div className="flex items-center gap-3 mb-4 px-1">
                    <span className="text-primary"><Icon name="cloud" size={18} /></span>
                    <h2 className="font-mono text-[10px] uppercase tracking-[0.2em] text-on-surface-variant/50">Upstream</h2>
                  </div>
                  <div className="grid gap-4 sm:grid-cols-2">
                    {repo.parent && (
                      <RepoCard 
                        repo={repo.parent as any} 
                        label="Parent" 
                        description="Direct upstream repository."
                      />
                    )}
                    {repo.source && repo.source.full_name !== repo.parent?.full_name && (
                      <RepoCard 
                        repo={repo.source as any} 
                        label="Root Source" 
              ...
              <section>
                <div className="flex items-center gap-3 mb-6 px-1">
                  <span className="text-secondary"><Icon name="fork" size={18} /></span>
                  <h2 className="font-mono text-[10px] uppercase tracking-[0.2em] text-on-surface-variant/50">Forks ({repo.forks_count})</h2>
                </div>
                ...
                {forks.length === 0 ? (
                  <div className="rounded-sm border border-dashed border-outline-variant/30 p-12 text-center">
                    ...
                    <p className="text-sm text-on-surface-variant/60">No forks found for this repository.</p>
                  </div>
                ) : (
    ...
    return (
      <>
        <TopBar repo={repoSlug} />
        <ErrorState title="Failed to load network" body={e.message || "Unable to fetch repository details."} />
      </>
    );
  }
}

function RepoCard({ repo, label, description }: { repo: any, label?: string, description?: string }) {
