          ...
          <aside className="hidden lg:flex w-72 shrink-0 flex-col bg-surface-container-low">
            <div className="px-5 py-4">
              ...
              <CompareSelector
                ...
                base={base!}
                head={head!}
                commits={recentCommits.map(c => ({ sha: c.sha, message: c.commit.message }))}
              />
    ...
    <div className="font-mono text-[12px] leading-[1.6]">
      {lines.map((l, i) => {
        ...
        if (l.kind === "hunk") {
          ...
          if (m) {
            leftN = parseInt(m[1], 10);
            rightN = parseInt(m[2], 10);
          }
          return (
            <div
              key={i}
        ...
        const left = !isAdd ? leftN++ : null;
        const right = !isDel ? rightN++ : null;
